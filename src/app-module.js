...
const App = (() => {
    ...
    editArray.forEach((prop) => {
      prop.addEventListener('click', (e) => {
        const task = taskHolder
          .getArray()
          .find((object) => object.position == e.target.attributes[0].value);
  ...
  const createRemoveEventListeners = () => {
    ...
    removeArray.forEach((prop) => {
      prop.addEventListener('click', (e) => {
        ...
        taskHolder.getArray().forEach((object, index) => {
  ...
  formElement.addEventListener('keydown', (e) => {
    if (e.key === 'Escape' && !viewBlocker.classList.contains('visibility')) {
      formVisibility();
    }
  });
  ...
  formElement.addEventListener('submit', (e) => {
    if (taskFormTitle.textContent === 'Add a task') {
      ...
      if (taskHolder.getArray().length === 0) {
        ...
        taskPosition = taskHolder.getArray().length;
      }
      ...
      form.reset();
      saveDataToLocal(taskHolder.getArray());
    } else if (taskFormTitle.textContent === 'Edit task') {
      e.preventDefault();

      const task = taskHolder
        .getArray()
        .find((object) => object.title === originalTitle);
